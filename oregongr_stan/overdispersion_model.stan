...
parameters {
    ...
    vector[N] obs;                  // unique intercept for each row in the data
    real<lower=0> sigma_obs;      // std dev of overdispersion in the data
}
transformed parameters {
    ...
    // linear model
    for(i in 1:N)
        mu[i] = a + obs[i]*sigma_obs + a_re[race_ethn_id[i]] + b_f*female[i] + b_el*eng_learn[i] +
...
model {
    ...
    // priors for the overdispersion
    sigma_obs ~ gamma(1.5, 1);
    obs ~ normal(0, 1);

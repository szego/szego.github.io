...
transformed parameters {
    ...
    // linear model
    for (i in 1:N)
        p[i] = a + a_re[race_ethn_id[i]] + b_f*female[i] + b_el*eng_learn[i] + 
                b_d*disability[i] + b_h*homeless[i] + b_ed*econ_disad[i];
}
model {
    int n_missing = 0;
    
    // fixed priors on the intercept and population effects
    a ~ normal(0, 4);
    b_f ~ normal(0, 2);
    b_el ~ normal(0, 2);
    b_d ~ normal(0, 2);
    ...
    for (i in 1:N)
        target += binomial_rsupp_lpmf(graduates[i] | cohort[i], p[i]);
}
generated quantities {
    ...
    vector[M] p_c;
    for (i in 1:M) {
        p_c[i] = a + a_re[c_race_ethn_id[i]] + b_f*c_female[i] + b_el*c_eng_learn[i] + 
                b_d*c_disability[i] + b_h*c_homeless[i] + b_ed*c_econ_disad[i];
        log_lik[i] = binomial_rsupp_lpmf(c_graduates[i] | c_cohort[i], p_c[i]);
    }
}
